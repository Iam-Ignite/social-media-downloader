...
export default function home() {
		...
		return (
			...
			<View className='bg-gray-100 flex-row w-full items-center justify-center rounded-md overflow-hidden  mt-5'>
				...
				<View className='p-4 gap-3'>
					...
					<TouchableOpacity onPress={() => downloadVideo(video.sd_url, video.id, video.image)} className=' overflow-hidden rounded-md '>
						<ImageBackground
					...
					<TouchableOpacity  onPress={() => downloadVideo(video.hd_url, video.id, video.image)} className='border border-[#FF543E] h-10 items-center justify-center overflow-hidden  rounded-md '>
	...
	return (
		<SafeAreaView className='flex-1 bg-white p-4 items-center'>
			...
			{layout ? (
				<View>
				<Text className='px-6 text-lg text-gray-800 font-semibold mb-5'>History by Applications</Text>
					...
					<View className=' rounded-md mx-4  mt-10 p-2 h-96'>
					<Text className='mb-5 font-semibold text-lg'>Downloads</Text>
				...
				<View>
					{data.map((video: any, index: any) => (
						<VideoDetails key={index} video={video} />
					))}
				</View>
			)}
			<DownloadModal setModalVisible={setModalVisible} progressState={progressState} modalVisible={modalVisible} />
		</SafeAreaView>
	);
}
